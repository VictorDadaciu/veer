...
error_code vk_command_pool::init(VkCommandPoolCreateFlags flags)
{
    VkCommandPoolCreateInfo command_pool_create_info{
        ...
        .flags = flags,
        .queueFamilyIndex = vk::context().device.queue.family
    };

    if (FAILED(vkCreateCommandPool(vk::context().device.vk, &command_pool_create_info, nullptr, &vk)))
...
{
    vkDestroyCommandPool(vk::context().device.vk, vk, nullptr);
}
...
std::expected<VkCommandBuffer, error_code> vk_command_pool::allocate_cmd_buffer(bool is_primary) const
{
    ...
    if (FAILED(vkAllocateCommandBuffers(vk::context().device.vk, &command_buffer_allocate_info, &cmd_buf)))
...
std::expected<std::vector<VkCommandBuffer>, error_code> vk_command_pool::allocate_cmd_buffers(bool is_primary, uint32_t n) const
{
    ...
    if (FAILED(vkAllocateCommandBuffers(vk::context().device.vk, &command_buffer_allocate_info, cmd_bufs.data())))
