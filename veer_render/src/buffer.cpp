...
namespace ve
{
error_code vk_buffer::upload(const std::byte* data, VkDeviceSize size)
{
...
error_code buffer::init(const std::byte* data, size_t size)
{
    destroy();
    cpu.data = data;
    cpu.size = size;
...
error_code buffer::init(const byte_span& data)
{
    destroy();
    cpu = data;
    return error_code::success;
...
error_code buffer::init(size_t size)
{
    destroy();
    cpu = byte_span{
